<x-layout>
    ...
    <script>
        ...
        // FUNGSI BARU UNTUK INPUT NAMA STAFF PENERIMA SAAT CEKLIS KEMBALI
        // FUNGSI BARU UNTUK DROPDOWN NAMA STAFF PENERIMA SAAT CEKLIS KEMBALI
        window.confirmComplete = function(formId) {
            // Mapping data user dari PHP ke format Objek Javascript
            const staffOptions = {
            ...
            Swal.fire({
                ...
                text: "Pilih nama staff arsip yang menerima dokumen pengembalian ini:",
                input: 'select',
                inputOptions: staffOptions, // Menggunakan opsi dari database
                inputPlaceholder: '-- Pilih Staff Penerima --',
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#10b981',
                cancelButtonColor: '#E5E7EB',
                confirmButtonText: 'Selesaikan Pengembalian',
                cancelButtonText: 'Batal',
                customClass: { cancelButton: 'text-gray-700 font-bold' },
                inputValidator: (value) => {
                    if (!value) {
                        return 'Silakan pilih nama staff penerima terlebih dahulu!';
                    }
                }
            }).then((result) => {
                if (result.isConfirmed) {
                    ...
                    // Buat elemen input hidden untuk dikirim ke Controller
                    const inputPenerima = document.createElement('input');
                    inputPenerima.type = 'hidden';
                    inputPenerima.name = 'staff_penerima';
                    inputPenerima.value = result.value;

                    form.appendChild(inputPenerima);
                    form.submit();
                }
            });
        };
            ...
            <div class="bg-white p-4 md:p-5 rounded-2xl shadow-sm border border-gray-100 mb-6">
                <form action="/peminjaman" method="GET" class="flex flex-col lg:flex-row gap-4 justify-between items-center" hx-disable>
                    <div class="relative w-full lg:w-96 group">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" onchange="this.form.submit()" placeholder="Cari peminjam, berkas, staf..."
                    ...
                    <div class="flex flex-wrap gap-3 w-full lg:w-auto items-center">
                        ...
                        <div x-data="{ open: false }" class="relative grow sm:flex-none min-w-40">
                            <button type="button" @click="open = !open" @click.outside="open = false" class="w-full bg-gray-50 hover:bg-red-50 border border-gray-200 text-left pl-4 pr-10 py-3 rounded-xl text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-[#e92027]/20 transition-all truncate shadow-sm">
                                {{ request('status', 'Semua Status') == 'All' ? 'Semua Status' : request('status', 'Semua Status') }} {!! $chevron !!}
                            </button>
                            <div x-show="open" style="display: none;" class="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl py-1 max-h-60 overflow-y-auto top-full left-0">
                                ...
                                <label class="block px-4 py-2.5 text-sm cursor-pointer hover:bg-gray-50"><input type="radio" name="status" value="{{ $val }}" onchange="this.form.submit()" class="hidden" {{ (request('status') == $val || (request('status') == '' && $val == 'All')) ? 'checked' : '' }}> {{ $lbl }}</label>
                                @endforeach
                            </div>
                        </div>
                            ...
                            <div class="relative" x-data="{ showSortDropdown: false }">
                                ...
                                <div x-show="showSortDropdown" style="display: none;" class="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-100 z-50 overflow-hidden" x-transition.opacity>
                                    <ul class="text-xs font-medium text-gray-600">
                                        <li><a href="{{ request()->fullUrlWithQuery(['sort' => 'latest_added']) }}" class="block px-4 py-3 hover:bg-red-50">Terbaru Ditambahkan</a></li>
                                        <li><a href="{{ request()->fullUrlWithQuery(['sort' => 'oldest_added']) }}" class="block px-4 py-3 hover:bg-red-50">Terlama Ditambahkan</a></li>
                                        <li><a href="{{ request()->fullUrlWithQuery(['sort' => 'latest_date']) }}" class="block px-4 py-3 hover:bg-red-50">Tgl Pinjam Terbaru</a></li>
                                        <li><a href="{{ request()->fullUrlWithQuery(['sort' => 'oldest_date']) }}" class="block px-4 py-3 hover:bg-red-50">Tgl Pinjam Terlama</a></li>
                                    </ul>
                                </div>
                            </div>
                    ...
                    <div class="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 relative">
                        <div class="flex justify-between items-start mb-3 border-b border-gray-100 pb-3">
                            <div>
                                ...
                                <div class="text-[11px] text-gray-500 bg-gray-100 px-2 py-0.5 mt-1 rounded inline-block">{{ $detail->peminjaman->jabatan_peminjam }}</div>
                            </div>
                        ...
                        <!-- Info Arsip -->
                        <div class="grid grid-cols-1 gap-2 bg-gray-50 p-3 rounded-xl border border-gray-100 mb-3">
                            <div>
                                <span class="text-[10px] text-gray-500 font-bold block uppercase mb-1">Arsip & Box</span>
                                <div class="text-xs font-bold text-[#e92027]">{{ $detail->arsip ? $detail->arsip->nama_berkas : $detail->nama_arsip }}</div>
                                <div class="text-xs text-gray-600 mt-1">Box: {{ ($detail->arsip && $detail->arsip->no_box) ? $detail->arsip->no_box : ($detail->no_box ?? '-') }}</div>
                            </div>
                        </div>

                        <!-- Info Pemberi/Penerima & Tanggal -->
                        <div class="grid grid-cols-2 gap-3 bg-red-50/30 p-3 rounded-xl border border-red-50 mb-3">
                            <div>
                                <span class="text-[9px] text-gray-400 font-bold block uppercase">Tgl Pinjam</span>
                                <div class="text-[11px] font-bold text-gray-800">{{ \Carbon\Carbon::parse($detail->peminjaman->tanggal_pinjam)->format('d M Y') }}</div>
                                <span class="text-[9px] text-gray-400 block mt-1">Pemberi:</span>
                                <div class="text-[11px] font-bold text-[#e92027] truncate">{{ $detail->peminjaman->staff_pemberi ?? '-' }}</div>
                            </div>
                            <div>
                                <span class="text-[9px] text-gray-400 font-bold block uppercase">Tgl Kembali</span>
                                <div class="text-[11px] font-bold text-gray-800">{{ $detail->peminjaman->tanggal_kembali ? \Carbon\Carbon::parse($detail->peminjaman->tanggal_kembali)->format('d M Y') : '-' }}</div>
                                <span class="text-[9px] text-gray-400 block mt-1">Penerima:</span>
                                <div class="text-[11px] font-bold text-green-700 truncate">{{ $detail->peminjaman->staff_penerima ?? '-' }}</div>
                            </div>
                        </div>
                        ...
                        <div class="pt-2 flex justify-between items-center mt-2">
                            <div class="flex gap-2">
                                @if($detail->peminjaman->bukti_peminjaman)
                                    ...
                                    <button @click="showFilesModal = true; selectedFiles = {{ json_encode($files) }}" class="px-3 py-1.5 text-white bg-blue-600 rounded-lg text-xs font-bold border border-blue-700"><i class="fas fa-file-pdf"></i> Bukti</button>
                                @endif
                            </div>
                            <div class="flex gap-1">
                                @if($detail->peminjaman->status == 'Sedang Dipinjam')
                                    <form id="complete-m-{{ $detail->peminjaman->id }}" action="/peminjaman/{{ $detail->peminjaman->id }}/complete" method="POST" hx-disable>
                                        @csrf @method('PATCH')
                                        <button type="button" onclick="confirmComplete('complete-m-{{ $detail->peminjaman->id }}')" class="p-2 bg-white text-green-600 rounded-lg shadow-sm border border-green-200 hover:bg-green-50"><i class="fas fa-check text-xs"></i> Selesaikan</button>
                                    </form>
                                    <!-- Tombol Edit Aktif -->
                                    <a href="/peminjaman/{{ $detail->peminjaman->id }}/edit" class="p-2 bg-white text-amber-500 rounded-lg shadow-sm border border-gray-200 hover:bg-amber-50"><i class="fas fa-pen text-xs"></i></a>
                                @else
                                    <!-- Tombol Edit Mati (Jika sudah kembali) -->
                                    <button disabled class="p-2 bg-gray-50 text-gray-300 rounded-lg border border-gray-100 cursor-not-allowed"><i class="fas fa-pen text-xs"></i></button>
                                @endif
                            </div>
                        </div>
                    </div>
                ...
                <table class="w-full text-left table-fixed">
                    <thead>
                        <tr class="bg-[#e92027] text-white uppercase text-[10px] tracking-wider border-b border-red-900/20">
                            <th class="px-4 py-4 w-10 text-center border-r border-red-900/20">No</th>
                            <th class="px-4 py-4 w-48 border-r border-red-900/20">Info Peminjam</th>
                            ...
                            <th class="px-4 py-4 w-32 border-r border-red-900/20 text-center">Tgl Kembali</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($peminjaman as $index => $detail)
                            <tr class="hover:bg-red-50/30 transition duration-150 group">
                                <td class="px-4 py-4 text-center border-r border-gray-100 font-bold text-gray-500 text-xs">
                                    {{ $peminjaman->firstItem() + $index }}
                                </td>
                                <td class="px-4 py-4 border-r border-gray-100">
                                    <div class="font-bold text-gray-800 text-sm truncate" title="{{ $detail->peminjaman->nama_peminjam }}">{{ $detail->peminjaman->nama_peminjam }}</div>
                                ...
                                <td class="px-4 py-4 border-r border-gray-100">
                                    <div class="font-bold text-[#e92027] text-sm truncate" title="{{ $detail->arsip ? $detail->arsip->nama_berkas : $detail->nama_arsip }}">{{ $detail->arsip ? $detail->arsip->nama_berkas : $detail->nama_arsip }}</div>
                                    <div class="mt-1 flex gap-2 items-center text-[10px]">
                                        ...
                                        <span class="text-gray-500 font-mono font-bold">Box: {{ ($detail->arsip && $detail->arsip->no_box) ? $detail->arsip->no_box : ($detail->no_box ?? '-') }}</span>
                                    </div>
                                ...
                                <!-- KOLOM TGL PINJAM -->
                                <td class="px-4 py-4 text-center border-r border-gray-100 bg-red-50/20">
                                    <div class="text-xs font-bold text-[#e92027] mb-1 truncate" title="{{ $detail->peminjaman->staff_pemberi }}">{{ $detail->peminjaman->staff_pemberi ?? '-' }}</div>
                                    <div class="text-[11px] font-bold text-gray-700 mb-2">{{ \Carbon\Carbon::parse($detail->peminjaman->tanggal_pinjam)->format('d M Y') }}</div>

                                    @if($detail->peminjaman->bukti_peminjaman)
                                        @php $files = is_array(json_decode($detail->peminjaman->bukti_peminjaman)) ? json_decode($detail->peminjaman->bukti_peminjaman) : [$detail->peminjaman->bukti_peminjaman]; @endphp
                                        <button @click="showFilesModal = true; selectedFiles = {{ json_encode($files) }}" class="px-2 py-1 bg-white text-blue-600 text-[10px] font-bold rounded border border-blue-200 hover:bg-blue-50 transition w-full"><i class="fas fa-file-pdf"></i> Lihat Bukti</button>
                                    @endif
                                </td>
                                <!-- KOLOM TGL KEMBALI -->
                                <td class="px-4 py-4 text-center border-r border-gray-100 bg-green-50/20">
                                    @if($detail->peminjaman->status == 'Sudah Dikembalikan' || $detail->peminjaman->status == 'Telah Dikembalikan')
                                        <div class="text-xs font-bold text-green-700 mb-1 truncate" title="{{ $detail->peminjaman->staff_penerima }}">{{ $detail->peminjaman->staff_penerima ?? '-' }}</div>
                                        <div class="text-[11px] font-bold text-gray-700">{{ $detail->peminjaman->tanggal_kembali ? \Carbon\Carbon::parse($detail->peminjaman->tanggal_kembali)->format('d M Y') : '-' }}</div>
                                    @else
                                        <span class="text-gray-300 text-[10px]">- Belum Kembali -</span>
                                    @endif
                                </td>
                                    ...
                                    <div class="flex justify-center items-center gap-1.5">
                                        @if($detail->peminjaman->status == 'Sedang Dipinjam')
                                            <form id="complete-d-{{ $detail->peminjaman->id }}" action="/peminjaman/{{ $detail->peminjaman->id }}/complete" method="POST" hx-disable>
                                                @csrf @method('PATCH')
                                                <button type="button" onclick="confirmComplete('complete-d-{{ $detail->peminjaman->id }}')" class="w-8 h-8 flex items-center justify-center bg-white text-green-600 rounded-lg hover:bg-green-50 transition shadow-sm border border-gray-200" title="Selesaikan Pengembalian"><i class="fas fa-check text-xs"></i></button>
                                            </form>
                                        ...
                                        @else
                                            <!-- Tombol Selesaikan Mati -->
                                            <button disabled class="w-8 h-8 flex items-center justify-center bg-gray-50 text-gray-300 rounded-lg border border-gray-100 cursor-not-allowed" title="Sudah Dikembalikan"><i class="fas fa-check text-xs"></i></button>

                                            <!-- Tombol Edit Mati (Jika sudah kembali) -->
                                            <button disabled class="w-8 h-8 flex items-center justify-center bg-gray-50 text-gray-300 rounded-lg border border-gray-100 cursor-not-allowed" title="Tidak dapat diedit karena sudah dikembalikan"><i class="fas fa-pen text-xs"></i></button>
                                        @endif
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="7" class="px-6 py-16 text-center text-gray-400 bg-gray-50/50"><i class="fas fa-file-invoice text-4xl mb-3 text-gray-300"></i><br>Tidak ada data arsip peminjaman.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
