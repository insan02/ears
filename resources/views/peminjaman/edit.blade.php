<x-layout>
    ...
    <!-- JSON DATA TERSEMBUNYI UNTUK DIBACA OLEH ALPINE -->
    <script type="application/json" id="data-arsip-edit">{!! json_encode($daftarArsip ?? []) !!}</script>
    ...
    <div class="bg-gradient-to-br from-[#e92027] via-[#b91c1c] to-[#7f090b] text-white pb-32 pt-16 px-8 -mt-6 -mx-6 mb-8 rounded-b-[3rem] shadow-2xl relative overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto relative z-10 text-center md:text-left">
            <h1 class="text-3xl md:text-4xl font-extrabold tracking-tight mb-2 drop-shadow-md">Edit Data Peminjaman</h1>
            <p class="text-red-50 text-base font-light opacity-95 max-w-lg leading-relaxed drop-shadow-sm">Perbarui informasi, daftar arsip, dan dokumen bukti peminjaman.</p>
        </div>
    </div>

    <!-- INLINE X-DATA MEMBUATNYA KEBAL DARI BUG HTMX -->
    <div class="max-w-5xl mx-auto px-4 md:px-6 -mt-20 relative z-20 mb-10"
         x-data="{
            daftarArsip: [], serverErrors: [],
            jabatan: '{{ addslashes($editData->jabatan_peminjam ?? '') }}',
            unit: '{{ addslashes($editData->unit_peminjam ?? '') }}',
            items: [], files: [{ id: Date.now(), name: null }], deleteOldFile: false,
            showModal: false, showValidationModal: false, searchQuery: '', openDropdown: false, editingIndex: null,
            tempItem: { source: 'db', id: null, display_name: '', nama_manual: '', no_box: '', akses: 'Biasa', media: 'Softfile', fisik: 'Berkas Asli' },

            init() {
                this.daftarArsip = JSON.parse(document.getElementById('data-arsip-edit').textContent || '[]');
                this.serverErrors = JSON.parse(document.getElementById('data-errors-edit').textContent || '[]');
                const rawItems = JSON.parse(document.getElementById('data-items-edit').textContent || '[]');
                ...
                if (this.serverErrors.length > 0) this.showValidationModal = true;
            },
            openModal(index = null) {
                this.editingIndex = index;
                if (index !== null) {
                    this.tempItem = {...this.items[index]};
                    this.searchQuery = this.tempItem.source === 'db' ? this.tempItem.display_name : '';
                } else {
            ...
            get filteredArsip() {
                ...
                return this.daftarArsip.filter(item => {
            ...
            removeItem(index) {
                ...
                else { this.serverErrors = ['Minimal harus ada 1 arsip!']; this.showValidationModal = true; }
            },
            ...
            submitForm(form) {
                ...
                if (this.deleteOldFile && !hasNewFile) {
                    this.serverErrors.push('Anda menghapus file lama. Wajib mengupload Bukti (PDF) baru!');
                    formValid = false;
                } else if (!this.deleteOldFile && !hasExistingFile && !hasNewFile) {
                    this.serverErrors.push('Wajib mengupload minimal 1 File Bukti Peminjaman (Format PDF)');
                    formValid = false;
                }
        ...
        <form action="/peminjaman/{{ $editData->id }}" method="POST" enctype="multipart/form-data" @submit.prevent="submitForm($el)" class="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden" novalidate hx-disable>
            @csrf @method('PUT')
            ...
            <div class="p-6 md:p-8 space-y-8">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="relative">
                                <select name="unit" x-model="unit" required class="w-full border border-gray-200 rounded-xl px-4 py-3 bg-gray-50 text-gray-800 outline-none appearance-none cursor-pointer focus:bg-white focus:border-[#e92027] transition">
                                    ...
                                    @foreach($units as $u) <option value="{{ $u->nama_unit }}" {{ $editData->unit_peminjam == $u->nama_unit ? 'selected' : '' }}>{{ $u->nama_unit }}</option> @endforeach
                                </select>
                                <div class="absolute inset-y-0 right-0 flex items-center px-4 pointer-events-none text-gray-500"><i class="fas fa-chevron-down text-sm"></i></div>
                            </div>
                        </div>
                        <div class="md:col-span-2"><label class="block text-sm font-bold text-gray-800 mb-2">Keperluan <span class="text-[#e92027]">*</span></label><textarea name="keperluan" rows="3" required class="w-full border border-gray-200 rounded-xl px-4 py-3 bg-gray-50 text-gray-800 focus:bg-white outline-none focus:border-[#e92027] transition">{{ $editData->keperluan }}</textarea></div>
                    </div>
                </div>
                ...
                <div class="bg-red-50/50 p-4 md:p-6 rounded-2xl border border-red-100">
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                        ...
                        <button type="button" @click="openModal()" class="w-full md:w-auto px-5 py-2.5 bg-[#e92027] text-white text-sm font-bold rounded-xl hover:bg-[#801010] shadow-md transition flex items-center justify-center gap-2"><i class="fas fa-plus-circle"></i> Tambah Arsip Lain</button>
                    </div>
                        ...
                        <template x-for="(item, index) in items" :key="index">
                            <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm relative">
                                <div class="flex justify-between items-start mb-2">
                                    ...
                                    <button type="button" @click="openModal(index)" class="text-yellow-600 hover:text-yellow-700 bg-yellow-50 rounded-lg p-1.5 absolute top-3 right-10"><i class="fas fa-pen"></i></button>
                                    <button type="button" @click="removeItem(index)" class="text-red-500 hover:text-red-700 bg-red-50 rounded-lg p-1.5 absolute top-3 right-2"><i class="fas fa-trash-alt"></i></button>
                                </div>
                        ...
                        <table class="w-full text-left table-fixed">
                            <thead class="bg-[#e92027] text-white">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-xs font-bold uppercase w-24 text-center">Aksi</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr class="hover:bg-red-50">
                                        ...
                                        <td class="px-4 py-3 text-center">
                                            <div class="flex justify-center gap-2">
                                                <button type="button" @click="openModal(index)" class="text-yellow-600 hover:text-white bg-white border border-yellow-200 hover:bg-yellow-500 w-8 h-8 rounded-lg flex items-center justify-center shadow-sm transition"><i class="fas fa-pen text-xs"></i></button>
                                                <button type="button" @click="removeItem(index)" class="text-[#e92027] hover:text-white bg-white border border-red-200 hover:bg-[#e92027] w-8 h-8 rounded-lg flex items-center justify-center shadow-sm transition"><i class="fas fa-trash-alt text-xs"></i></button>
                                            </div>
                                        </td>
                                    </tr>
                                </template>
                ...
                <div>
                    ...
                    @if($editData->bukti_peminjaman)
                        @php $existingFiles = is_array(json_decode($editData->bukti_peminjaman)) ? json_decode($editData->bukti_peminjaman) : [$editData->bukti_peminjaman]; @endphp
                        <div x-show="!deleteOldFile" class="mb-6 bg-gray-50 border border-gray-200 rounded-xl p-4 transition-all">
                            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-3 gap-2">
                                <div><p class="text-xs font-bold text-gray-800 uppercase tracking-wide">File Tersimpan Saat Ini</p></div>
                                <button type="button" @click="deleteOldFile = true" class="w-full md:w-auto text-xs font-bold text-[#e92027] flex items-center justify-center gap-1 bg-white px-3 py-1.5 rounded border border-red-200 hover:bg-red-50 transition"><i class="fas fa-trash-alt"></i> Hapus Semua Lama</button>
                            </div>
                        ...
                        <div x-show="deleteOldFile" class="mb-6 bg-red-50 border border-red-200 rounded-xl p-4 flex justify-between items-center" style="display:none;">
                            ...
                            <button type="button" @click="deleteOldFile = false" class="text-xs font-bold text-gray-600 bg-white px-3 py-1.5 rounded border border-gray-300 shadow-sm">Batal Hapus</button>
                        </div>
                    @endif
            ...
            <div class="bg-gray-50 px-6 py-5 border-t border-gray-100 flex justify-end gap-3 rounded-b-3xl">
                ...
                <button type="submit" class="px-8 py-3 bg-[#e92027] text-white rounded-xl font-bold shadow-lg hover:bg-[#a0131a] transition transform hover:-translate-y-0.5">Perbarui Peminjaman</button>
            </div>
        </form>
            ...
            <div class="bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl overflow-visible border-t-8 border-[#e92027] relative" @click.away="closeModal()">
                <div class="p-6 md:p-8 space-y-6">
                    <div class="flex justify-between items-center mb-2">
                        ...
                        <button type="button" @click="closeModal()" class="text-gray-400 hover:text-[#e92027]"><i class="fas fa-times text-xl"></i></button>
                    </div>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                        ...
                        <button type="button" @click="addItem()" class="px-6 py-3 bg-[#e92027] text-white rounded-xl font-bold hover:bg-[#a0131a] shadow-md transition" x-text="editingIndex !== null ? 'Ubah Item' : 'Simpan Item'"></button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div x-show="showValidationModal" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div @click.away="showValidationModal = false" class="bg-white rounded-[2rem] w-full max-w-sm p-8 text-center relative overflow-hidden shadow-2xl border-t-8 border-[#e92027]">
                ...
                <button type="button" @click="showValidationModal = false" class="w-full py-3.5 bg-[#e92027] text-white rounded-xl text-sm font-bold hover:bg-[#801010] shadow-lg transition">OK, Saya Mengerti</button>
            </div>
        </div>
    </div>
</x-layout>
