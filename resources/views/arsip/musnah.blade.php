    ...
    <div class="bg-gray-50 min-h-screen pb-20">
        ...
        <div class="max-w-7xl mx-auto px-4 -mt-12 md:-mt-20 relative z-20 mb-12 print:mt-0 print:px-0">
            <div class="bg-white rounded-2xl md:rounded-3xl shadow-xl overflow-hidden border border-gray-100 min-h-[400px] flex flex-col print:shadow-none print:border-0 print:p-0">
                ...
                <div class="block lg:hidden space-y-4 p-4 bg-gray-50/50 flex-grow">
                ...
                <div class="hidden lg:block flex-grow w-full p-6">
                        ...
                        <table class="w-full text-left border-collapse table-fixed">
                            <thead>
                                <tr class="bg-gray-800 text-white uppercase tracking-wider text-[9px]">
                                    ...
                                    <th class="py-3 px-3 font-bold w-[25%] border-r border-gray-700">Uraian</th>
                                    ...
                                    <th class="py-3 px-2 font-bold text-center w-[10%] border-r border-gray-700">Status</th>
                            ...
                            <tbody class="text-gray-600 text-[10px] divide-y divide-gray-100">
                                @forelse($arsips as $index => $arsip)
                                    <tr class="hover:bg-gray-50 transition duration-200">
                                        <td class="py-2 px-2 text-center font-bold border-r border-gray-100">{{ $arsips->firstItem() + $index }}</td>
                                        <td class="py-2 px-2 text-center border-r border-gray-100 font-bold text-gray-700 truncate" title="{{ $arsip->klasifikasi->kode_klasifikasi ?? '-' }}">
                                            {{ $arsip->klasifikasi->kode_klasifikasi ?? '-' }}
                                        </td>
                                        <td class="py-2 px-3 font-bold text-gray-800 border-r border-gray-100 truncate" title="{{ $arsip->nama_berkas }}">
                                            {{ $arsip->nama_berkas }}
                                        </td>
                                        <td class="py-2 px-3 text-gray-500 border-r border-gray-100 truncate" title="{{ $arsip->isi }}">
                                            {{ $arsip->isi }}
                                        </td>
                                        <td class="py-2 px-2 text-center font-medium border-r border-gray-100">{{ $arsip->tahun ?? '-' }}</td>
                                        <td class="py-2 px-2 text-center border-r border-gray-100">
                                            {{ $arsip->tanggal_masuk ? \Carbon\Carbon::parse($arsip->tanggal_masuk)->format('d/m/y') : '-' }}
                                        </td>
                                        <td class="py-2 px-2 text-center font-bold border-r border-gray-100">{{ $arsip->jumlah ?? '-' }}</td>
                                        <td class="py-2 px-2 text-center font-mono font-bold text-gray-700 border-r border-gray-100 truncate" title="{{ $arsip->no_box }}">
                                            {{ $arsip->no_box ?? '-' }}
                                        </td>
                                        <td class="py-2 px-2 text-center border-r border-gray-100">
                                            <span class="font-bold uppercase truncate" title="{{ $arsip->tindakan_akhir }}">
                                                {{ $arsip->tindakan_akhir ?? '-' }}
                                            </span>
                                        </td>
                                        <td class="py-2 px-2 text-center text-[#e92027] font-mono font-bold bg-red-50/50">
