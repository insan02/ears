<x-layout>
    <!-- SCRIPT ALPINE KHUSUS EDIT DI ATAS -->
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
    <script>
        window.editArsipForm = function() {
            return {
                unitPengolah: @json($arsip->unit_pengolah ?? ''),
                kodeKlasifikasi: @json($arsip->klasifikasi->kode_klasifikasi ?? ''),
                klasifikasiId: @json($arsip->klasifikasi_id ?? ''),
                hakAkses: @json($arsip->hak_akses ?? '-'),
                masaSimpan: @json($arsip->masa_simpan ?? '-'),
                tindakanAkhir: @json($arsip->tindakan_akhir ?? '-'),

                validateEditSubmit(event) {
                    const namaBerkas = document.querySelector('[name=nama_berkas]').value;
                    const isiBerkas = document.querySelector('[name="isi_berkas[0][isi]"]').value;
                    ...
                    if (errorMessages.length > 0) {
                        ...
                        document.getElementById('formEditArsip').submit();
                    }
                },
                updateDefaults(data) {
        ...
        window.classificationDropdown = function(initData) {
            return {
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" x-data="editArsipForm()" @classification-selected.window="updateDefaults($event.detail)">
        ...
        <div class="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden flex flex-col">
            <form id="formEditArsip" action="{{ route('arsip.update', $arsip->id) }}" method="POST" @submit.prevent="validateEditSubmit($event)" class="flex flex-col flex-1" hx-disable>
                ...
                {{-- Header --}}
                <div class="px-8 py-6 border-b border-gray-50 bg-gradient-to-r from-amber-600 to-red-800 flex justify-between items-center shrink-0">
                    <div class="flex items-center gap-4">
                        <div class="p-3 bg-white/25 rounded-xl backdrop-blur-sm text-white shadow-inner">
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                        </div>
                        <div>
                            <h1 class="text-2xl font-black text-white tracking-tight">Edit Arsip Dokumen</h1>
                ...
                <div class="p-8 bg-gray-50/50 space-y-8">
                    ...
                    <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        {{-- Klasifikasi Dropdown (Dengan Fungsi Sama Persis Seperti Create) --}}
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div class="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center justify-between">
                            ...
                            <input type="hidden" name="isi_berkas[0][hak_akses]" x-model="hakAkses">
                        ...
                        <div class="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center justify-between">
                            ...
                            <input type="hidden" name="isi_berkas[0][masa_simpan]" x-model="masaSimpan">
                        ...
                        <div class="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center justify-between">
                            ...
                            <input type="hidden" name="isi_berkas[0][tindakan_akhir]" x-model="tindakanAkhir">
