...
<div x-data="{ showDeleteModal: false, deleteUrl: '', isDeleting: false }" class="relative">
    ...
    <div class="block xl:hidden space-y-4">
        @forelse ($arsips as $arsip)
            @php
                $isMerged = ($arsip->no_berkas === '__MERGED_ROW__');
                $mergedText = $arsip->nama_berkas;
                    ...
                    <div class="flex justify-between items-start mb-3 border-b border-gray-100 pb-3">
                        <div class="pr-8">
                            <div class="flex items-center gap-2 mb-1">
                                @if($arsip->no_berkas && $arsip->no_berkas !== '-')
                        ...
                        <tr class="group hover:bg-red-50/40 transition-all duration-200 text-[10px]">
                            ...
                            <td class="py-2 px-1 text-center align-top">
                                <div class="flex justify-center items-center gap-1 mt-1">
                                    ...
                                    @if(str_contains($tindakanLower, 'musnah'))
                                        <button type="button" @click="deleteUrl = '{{ route('arsip.destroy', $arsip->id) }}'; showDeleteModal = true; isDeleting = false" class="w-6 h-6 flex items-center justify-center bg-white text-[#e92027] rounded border border-gray-200 hover:bg-red-50" title="Musnahkan">
    ...
    <div x-show="showDeleteModal" style="display: none;" class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
        ...
        <div class="bg-white rounded-3xl w-full max-w-sm p-8 text-center relative z-10 shadow-2xl">
            ...
            <div class="flex flex-col gap-3">
                <form :action="deleteUrl" method="POST" hx-disable @submit="isDeleting = true">
                    @csrf @method('DELETE')
                    <button type="submit" :disabled="isDeleting"
                        class="w-full py-3.5 bg-[#e92027] text-white rounded-xl text-sm font-bold shadow-md transition flex justify-center items-center gap-2"
                        :class="isDeleting ? 'opacity-70 cursor-wait' : 'hover:bg-[#c41820] transform hover:-translate-y-0.5'">
                        <span x-show="!isDeleting">Ya, Musnahkan</span>
                ...
                <button type="button" @click="showDeleteModal = false" :disabled="isDeleting"
                    class="w-full py-3.5 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold text-gray-600 transition"
