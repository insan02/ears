<x-layout>
    ...
    {{-- Main Card Container dengan Inline x-data --}}
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8"
         x-data="{
            formStep: 1,
            isiBerkas: [],
            newIsi: '', newTahun: '', newTanggal: '', newJumlah: 1,
            newNoBox: '', newHakAkses: '', newMedia: '', newMasaSimpan: '', newTindakan: 'Musnah',
            namaBerkas: '', unitPengolah: '', kodeKlasifikasi: '', klasifikasiId: '',

            addIsi() {
                if (this.newIsi.trim() !== '') {
                    this.isiBerkas.push({
                        isi: this.newIsi.trim(), tahun: this.newTahun, tanggal: this.newTanggal,
                        jumlah: this.newJumlah, no_box: this.newNoBox, hak_akses: this.newHakAkses,
                        jenis_media: this.newMedia || 'Kertas', masa_simpan: this.newMasaSimpan,
                        tindakan_akhir: this.newTindakan, unit_pengolah: this.unitPengolah,
                        kode_klasifikasi: this.kodeKlasifikasi, klasifikasi_id: this.klasifikasiId
                    });
                    this.newIsi = ''; this.newTahun = ''; this.newTanggal = '';
                    this.newJumlah = 1; this.newNoBox = ''; this.newMedia = '';
                    this.unitPengolah = ''; this.kodeKlasifikasi = ''; this.klasifikasiId = '';
                    this.newHakAkses = ''; this.newMasaSimpan = ''; this.newTindakan = 'Musnah';
            ...
            editItem(index) {
                ...
                this.newIsi = item.isi; this.newTahun = item.tahun; this.newTanggal = item.tanggal;
                this.newJumlah = item.jumlah; this.newNoBox = item.no_box; this.newHakAkses = item.hak_akses;
                this.newMedia = item.jenis_media; this.newMasaSimpan = item.masa_simpan;
                this.newTindakan = item.tindakan_akhir; this.unitPengolah = item.unit_pengolah;
                this.kodeKlasifikasi = item.kode_klasifikasi; this.klasifikasiId = item.klasifikasi_id;
                this.$dispatch('set-classification', {
                    code: item.kode_klasifikasi, id: item.klasifikasi_id, label: item.kode_klasifikasi,
                    hak_akses: item.hak_akses, masa_simpan: item.masa_simpan, tindakan: item.tindakan_akhir
                });
                this.isiBerkas.splice(index, 1);
            },
            removeIsi(index) { this.isiBerkas.splice(index, 1); },

            validateStep1() {
                if (!this.namaBerkas || this.namaBerkas.trim() === '') {
                    Swal.fire({ icon: 'warning', title: 'Data Belum Lengkap!', text: 'Mohon isi Nama Berkas terlebih dahulu.', confirmButtonColor: '#e92027' });
                    return;
                }
                this.formStep = 2;
            },
            ...
            validateFinalSubmit(event) {
                let errorMessages = [];
                if (!this.namaBerkas || this.namaBerkas.trim() === '') errorMessages.push('<b>Nama Berkas</b> masih kosong.');
                if (this.isiBerkas.length === 0) errorMessages.push('<b>Daftar Isi Berkas</b> masih kosong. Silakan isi Rincian dan klik Tambah ke Daftar.');

                if (errorMessages.length > 0) {
                    Swal.fire({ icon: 'error', title: 'Tidak Bisa Disimpan!', html: '<ul class=\'list-disc pl-5 text-left text-sm mt-2\'><li>' + errorMessages.join('</li><li>') + '</li></ul>', confirmButtonColor: '#e92027' });
                } else {
                    document.getElementById('formInputArsip').submit();
                }
            },
            updateDefaults(data) {
                this.newHakAkses = data.hak_akses || ''; this.newMasaSimpan = data.masa_simpan || '';
                this.newTindakan = data.tindakan || ''; this.kodeKlasifikasi = data.code || ''; this.klasifikasiId = data.id || '';
            }
        ...
        <div class="mb-6 bg-red-100 border border-red-400 text-red-700 px-6 py-4 rounded-xl shadow-lg flex items-center gap-4">
            ...
            <div>
                <strong class="font-bold">Terjadi Kesalahan Server!</strong>
        ...
        <div class="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden flex flex-col min-h-[600px]">
            ...
            <form id="formInputArsip" action="{{ route('arsip.store') }}" method="POST" @submit.prevent="validateFinalSubmit($event)" class="flex flex-col flex-1 h-full" hx-disable>
                ...
                <div class="flex-1 overflow-y-auto overflow-x-hidden p-8 bg-gray-50/50">
                    ...
                    {{-- STEP 1 --}}
                    <div x-show="formStep === 1" x-transition:enter="transition ease-[cubic-bezier(0.34,1.56,0.64,1)] duration-500">
                        <div class="max-w-xl mx-auto py-12 space-y-12">
                            <div class="text-center space-y-4">
                                <div class="inline-flex items-center justify-center w-20 h-20 bg-red-50 rounded-3xl text-red-600 mb-2 shadow-sm">
                            ...
                            <div class="space-y-8 bg-white p-8 rounded-[2rem] shadow-xl border border-gray-100/50">
                                ...
                                <div class="group relative">
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="text" value="{{ $nextNumber ?? 'AUTO' }}" disabled class="w-full pl-12 pr-6 py-5 border-2 border-gray-100 rounded-2xl bg-gray-50/50 text-gray-800 font-bold text-lg shadow-sm cursor-not-allowed">
                                ...
                                {{-- Nama Berkas --}}
                                <div class="group relative">
                                    <label class="block font-bold text-gray-700 mb-3 pl-2 text-sm uppercase tracking-wide group-focus-within:text-red-600 transition-colors">Nama Berkas <span class="text-red-500">*</span></label>
                    ...
                    {{-- STEP 2 --}}
                    <div x-show="formStep === 2" style="display: none;" x-transition:enter="transition ease-out duration-300">
                        <div class="space-y-8">
                            ...
                            <div class="bg-gray-50/50 p-6 rounded-3xl border border-gray-100">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                    <div class="group">
                                        ...
                                        <select x-model="unitPengolah" class="w-full p-4 border-2 border-transparent bg-white rounded-2xl focus:border-red-500 focus:ring-4 focus:ring-red-50 outline-none transition shadow-sm font-bold text-gray-800 appearance-none cursor-pointer">
                                    ...
                                    {{-- Kode Klasifikasi dengan Inline Data --}}
                                    <div class="relative group" @reset-selection.window="reset()" @set-classification.window="setFromParent($event.detail)"
                                         x-data="{
                                            open: false, step: 1, breadcrumbs: [], options: [], loading: false,
                                            selectedItem: null, displayText: 'Pilih Kode Klasifikasi',
                                            init() { this.fetchOptions(1); },
                                            toggle() {
                                                this.open = !this.open;
                                                if(this.open) {
                                            ...
                                            reset() {
                                                this.step = 1; this.breadcrumbs = []; this.selectedItem = null;
                                                this.displayText = 'Pilih Kode Klasifikasi'; this.fetchOptions(1); this.open = false;
                                            },
                                            setFromParent(detail) {
                                                this.selectedItem = { code: detail.code, label: detail.label, id: detail.id };
                                                this.displayText = detail.code; this.open = false; this.step = 1; this.breadcrumbs = [];
                                            }
                                         }">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
                                    ...
                                    <div class="md:col-span-3 space-y-1">
                                        ...
                                        <select x-model="newMedia" @keydown.enter.prevent="addIsi()" class="w-full p-3 border-2 border-transparent bg-white rounded-xl focus:border-red-500 outline-none transition shadow-sm text-sm">
                                            <option value="" disabled selected>Pilih...</option><option value="Kertas">Kertas</option><option value="Foto">Foto</option><option value="Kartografi">Kartografi</option>
                                        </select>
                                    </div>
                                    <div class="md:col-span-2 space-y-1"><label class="text-xs font-bold text-gray-500 uppercase ml-1">Box</label><input type="text" x-model="newNoBox" @keydown.enter.prevent="addIsi()" placeholder="Box 1" class="w-full p-3 border-2 border-transparent bg-white rounded-xl focus:border-red-500 outline-none transition shadow-sm text-sm text-center font-bold"></div>
                                    <div class="md:col-span-2 space-y-1"><label class="text-xs font-bold text-gray-500 uppercase ml-1">Tahun</label><input type="number" x-model="newTahun" @keydown.enter.prevent="addIsi()" placeholder="YYYY" class="w-full p-3 border-2 border-transparent bg-white rounded-xl focus:border-red-500 outline-none transition shadow-sm text-sm text-center"></div>
                                    <div class="md:col-span-3 space-y-1"><label class="text-xs font-bold text-gray-500 uppercase ml-1">Tanggal</label><input type="date" x-model="newTanggal" @keydown.enter.prevent="addIsi()" class="w-full p-3 border-2 border-transparent bg-white rounded-xl focus:border-red-500 outline-none transition shadow-sm text-sm text-center text-gray-600"></div>
                                    <div class="md:col-span-2 space-y-1"><label class="text-xs font-bold text-gray-500 uppercase ml-1">Jml</label><input type="number" x-model="newJumlah" placeholder="1" min="1" class="w-full p-3 border-2 border-transparent bg-white rounded-xl focus:border-red-500 outline-none transition shadow-sm text-sm text-center font-bold"></div>
                                </div>
                                                ...
                                                <template x-for="(item, index) in isiBerkas" :key="index">
                                                    <tr class="hover:bg-red-50/30 transition-colors">
                                                        <td class="px-4 py-3">
                                                            ...
                                                            <div class="font-bold text-gray-800 truncate" x-text="namaBerkas"></div>
                                                        </td>
                                                        <td class="px-4 py-3 text-gray-600 line-clamp-2" x-text="item.isi"></td>
                                                        ...
                                                        <td class="px-4 py-3 text-center">
                                                            <div class="text-[9px] font-bold px-2 py-0.5 rounded-full border mb-1 truncate bg-gray-50 text-gray-600" x-text="'Akses: '+item.hak_akses"></div>
                                                            <div class="text-[9px] font-bold px-2 py-0.5 rounded-full border truncate bg-gray-50 text-gray-600" x-text="'Tndkn: '+item.tindakan_akhir"></div>
                                                        </td>
                                                        <td class="px-4 py-3 text-center">
                                                            <div class="font-bold text-gray-700 text-[10px] truncate uppercase mb-1" x-text="item.unit_pengolah"></div>
                                                ...
                                                <tr x-show="isiBerkas.length === 0">
                                                    <td colspan="6" class="px-6 py-16 text-center">
                                                        <div class="flex flex-col items-center text-gray-300">
                                                            <i class="fas fa-folder-open text-4xl mb-3"></i><p class="text-sm font-medium">Belum ada data arsip.</p>
                                                        </div>
                                                    </td>
                                                </tr>
                                            </tbody>
                            ...
                            {{-- Footer / Submit --}}
                            <div class="p-6 bg-white border-t border-gray-100 flex justify-between items-center gap-4 rounded-3xl shadow-lg mt-6">
