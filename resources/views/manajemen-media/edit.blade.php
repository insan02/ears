    ...
    <div x-data="{
        ...
        <div class="bg-gradient-to-br from-[#e92027] via-[#b91c1c] to-[#7f090b] px-4 md:px-8 pt-8 md:pt-12 pb-24 md:pb-32 rounded-b-[2rem] md:rounded-b-[3rem] shadow-xl mb-8 -mt-4 md:-mt-6 -mx-4 md:-mx-6 relative overflow-hidden">
            <div class="relative z-10 max-w-4xl mx-auto text-center md:text-left">
                <h1 class="text-2xl md:text-4xl font-extrabold text-white tracking-wide">Edit Berita / Media</h1>
                <p class="text-red-100 text-sm md:text-base mt-2 opacity-90 font-light">Perbarui data atau ganti gambar konten informasi.</p>
            </div>
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 -mt-16 md:-mt-24 relative z-20 mb-12">
            ...
            <form action="{{ route('manajemen-media.update', $media->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl md:rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
                @csrf
                @method('PUT')
                ...
                <div class="p-6 md:p-8 space-y-6 md:space-y-8">
                    <div>
                        <h2 class="text-base md:text-lg font-bold text-[#e92027] border-b border-gray-100 pb-3 mb-6 flex items-center gap-3">
                            <i class="fas fa-edit text-[#e92027]"></i> Update Konten
                        </h2>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="judul" id="judul" value="{{ old('judul', $media->judul) }}" required
                            ...
                            <div>
                                ...
                                <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', $media->tanggal) }}" required
                            ...
                            <!-- Gambar -->
                            <div>
                                <label for="gambar" class="block text-sm font-bold text-gray-800 mb-2">Ganti Gambar (Opsional)</label>
                                <!-- Menggunakan event bawaan Alpine: @change -->
                                <input type="file" name="gambar" id="gambar" accept="image/jpeg, image/png" @change="validateImage($event)"
                                ...
                                <p class="text-[11px] text-orange-500 mt-0.5">Abaikan jika tidak ingin mengubah gambar.</p>
                            </div>

                            <!-- Preview Gambar Lama -->
                            @if($media->gambar)
                            <div class="md:col-span-2 bg-gray-50 p-4 rounded-xl border border-gray-200 flex flex-col md:flex-row items-center gap-4">
                                <div class="shrink-0">
                                    <p class="text-xs text-gray-500 font-bold mb-2">Gambar Terpasang Saat Ini:</p>
                                    <img src="{{ asset($media->gambar) }}" alt="Preview" class="h-24 w-auto rounded-lg shadow-sm border border-white">
                                </div>
                            </div>
                            @endif
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="deskripsi" id="deskripsi" rows="6" required
                                    ...
                                    placeholder="Tulis deskripsi berita di sini...">{{ old('deskripsi', $media->deskripsi) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
