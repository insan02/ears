<x-layout>
        ...
        <div class="max-w-7xl mx-auto px-4 -mt-20 relative z-20 mb-12">
            ...
            <div class="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100 mb-8 p-4 md:p-6">
                <form action="{{ route('monitoring.index') }}" method="GET" class="flex flex-col lg:flex-row gap-4 justify-between items-center">
                    ...
                    <div class="flex gap-3 w-full lg:w-auto overflow-x-auto pb-2 lg:pb-0 hide-scrollbar scroll-smooth">
                ...
                <div x-data="{ tab: 'proses' }" class="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-200 mb-10">
                    ...
                    <div x-show="tab === 'proses'" x-transition.opacity>
                        ...
                        @else
                            ...
                            <!-- MOBILE VIEW (CARD) -->
                            <!-- Menggunakan md:hidden agar di HP menjadi card -->
                                        ...
                                        <div class="pt-3 flex flex-wrap gap-2 justify-between items-center border-t border-gray-100">
                                            <div class="flex items-center gap-2">
                                                ...
                                                @if(Auth::user()->role === 'admin' || Auth::id() == $item->user_id)
                                                    @if(!in_array($item->status_kerja, ['Selesai', 'Menunggu Alih Media', 'Menunggu E-Arsip']))
                                            ...
                                            <div class="flex items-center gap-2 mt-2 w-full">
                                                <form id="advance-form-m-{{ $item->id }}" action="{{ route('monitoring.advance-stage', $item->id) }}" method="POST" class="flex-grow">
                                                    @csrf @method('PATCH')
                                                    @if(in_array($item->status_kerja, ['Menunggu Alih Media', 'Menunggu E-Arsip']))
                            ...
                            <!-- DESKTOP VIEW (TABLE) -->
                            <!-- Menggunakan hidden md:block agar di Laptop jadi Tabel -->
                                            ...
                                            <tr class="hover:bg-red-50/20 transition duration-200 group text-sm">
                                                ...
                                                <td class="py-4 px-4 text-center">
                                                    <div class="flex items-center justify-center gap-1">
                                                        @if(!in_array($item->status_kerja, ['Menunggu Alih Media', 'Menunggu E-Arsip']) && (Auth::user()->role === 'admin' || Auth::id() == $item->user_id))
    ...
    <!-- MODAL HISTORY (Desain Mobile Responsif menggunakan List Card) -->
    <div id="historyModal" class="fixed inset-0 z-[100] hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl w-full relative z-[110] border border-gray-100">
                ...
                <!-- Container untuk List Card Riwayat -->
                <div class="px-6 py-6 max-h-[60vh] overflow-y-auto bg-gray-50/50 custom-scrollbar" id="historyContainer">
                    <!-- Diisi oleh AJAX (JS) -->
                </div>
    ...
    <script>
        ...
        window.confirmAdvance = function(formId, currentStage) {
            ...
            if(currentStage === 'Pemilahan') { nextStage = 'Pendataan'; confirmText = 'Aktivitas staf ini akan dipindahkan ke tahap Pendataan. Data Pemilahan akan dikunci sebagai riwayat.'; }
            else if(currentStage === 'Pendataan') { nextStage = 'Pelabelan'; confirmText = 'Aktivitas staf ini akan dipindahkan ke tahap Pelabelan. Data Pendataan akan dikunci sebagai riwayat.'; }
        ...
        // PERUBAHAN BESAR: Log Modal sekarang menggunakan List Card agar tidak perlu scroll ke samping di HP
